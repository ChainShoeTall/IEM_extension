...
# muti-preprocess process count: line 391
...
class BaseLoader(Dataset):
    ...
    def __init__(self, dataset_name, raw_data_path, config_data):
        ...
        """
        # additions
        self.specific = config_data.SPECIFIC
        self.isexcept = config_data.EXCEPT
        self.preprocess_multi_process_quota = config_data.PREPROCESS.MULTI_PROCESS_QUOTA
        self.fps = config_data.FS # To calculate timestamps
        ...
        # mod
        self.cached_path = config_data.CACHED_PATH # don't do it here, to config.py
        self.file_list_path = config_data.FILE_LIST_PATH # DataFileLists/*.csv
        self.preprocessed_data_len = 0
        self.data_format = config_data.DATA_FORMAT
        self.do_preprocess = config_data.DO_PREPROCESS
        self.raw_data_dirs = sorted(self.get_raw_data(self.raw_data_path), key=lambda x:x['index'])
        ...
        if config_data.DO_PREPROCESS:
            ...
            if not os.path.exists(self.cached_path):
                raise ValueError('Please set DO_PREPROCESS to True. Preprocessed directory does not exist!')
        ...
        # print(f"{self.dataset_name} Sample Number: {self.preprocessed_data_len} [BEGIN:{config_data.BEGIN}, END:{config_data.END}, EXCEPT:{config_data.EXCEPT}]", end='\n\n')
    ...
    def __getitem__(self, index):
        """
        Returns: 
            A clip of video(3,T,W,H)
            Its corresponding PPG signals(T)
            Prefix file name
            Chunk index
        """
        ...
        split_idx = item_path_filename.rindex('_') # string.rindex(target_str): find the last occurrence of target_str in string, return the index
        ...
        chunk_id = item_path_filename[split_idx + 6:].split('.')[0] # +6 will skip "_input", which is in total 6 chars
    ...
    def split_raw_data(self, data_dirs, begin, end):
        ...
        self.isexcept: True -> Include [begin, end];
                       False-> Exclude [begin, end];
        ...
        """
        if begin == 0 and end == 1 and not self.isexcept:  # return the full directory
            return data_dirs
        ...
        if self.isexcept:
            for i in range(0, int(begin * file_num)):
                data_dirs_new.append(data_dirs[i])
            for i in range(int(end * file_num), file_num):
                data_dirs_new.append(data_dirs[i])
        else:
            for i in range(int(begin * file_num), int(end * file_num)):
                data_dirs_new.append(data_dirs[i])
    ...
    def preprocess_dataset(self, data_dirs, config_preprocess, begin, end):
        ...
        file_list_dict = self.multi_process_manager(data_dirs_split, config_preprocess, 1)
    ...
    def face_detection(self, frame, use_larger_box=False, larger_box_coef=1.0):
        """Face detection on a single frame with mediapipe.
        ...
        my_face_detection = mp_face_detection.FaceDetection(model_selection=1, min_detection_confidence=0.5)
        # results = my_face_detection.process(cv2.cvtColor(bh_video[0].astype(np.uint8), cv2.COLOR_BGR2RGB)
        results = my_face_detection.process(frame) # cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)

        if not results.detections:
            print(r"ERROR: No Face Detected, take the center 60% regions as face")
            # face_box_coor = [0, 0, frame.shape[0], frame.shape[1]]
            # Assume the face is in the center 60%
            face_box_coor = [frame.shape[0]*0.2, frame.shape[1]*0.2, frame.shape[0]*0.6, frame.shape[1]*0.6]
        else:
            if len(results.detections) >= 2:
                print("Warning: More than one faces are detected(Only cropping the biggest one.)")
            rbb = results.detections[0].location_data.relative_bounding_box
            ih, iw, ic = frame.shape
            face_box_coor = [rbb.xmin * iw, rbb.ymin * ih, rbb.width * iw, rbb.height * ih]
        if use_larger_box:
    ...
    def face_crop_and_resize(self, frames, use_dynamic_detection, detection_freq, width, height, use_larger_box, use_face_detection, larger_box_coef):
        ...
        if use_dynamic_detection:
            ...
            num_dynamic_det = 1 # Once for one image stream
        ...
        options = FaceDetectorOptions(
            base_options=BaseOptions(model_asset_path='./utils/detector.tflite'),
            min_detection_confidence = 0.5,
            running_mode=VisionRunningMode.IMAGE)
        with FaceDetector.create_from_options(options) as detector:
            for idx in range(num_dynamic_det):
                mp_image = mp.Image(image_format = mp.ImageFormat.SRGB, data = frames[detection_freq * idx])
                try:
                    detection_result = detector.detect_for_video(mp_image,int(1000*idx/self.fps))
                    face_region_all.append(detection_result.detections[0].bounding_box)
                except:
                    face_region_all.append(None)
        ...
        for i in range(0, frames.shape[0]):
            ...
            if use_face_detection:
                ...
                frame = frame[max(face_region.y, 0):min(face_region.y + face_region.height, frame.shape[0]),
                        max(face_region.x, 0):min(face_region.x + face_region.width, frame.shape[1])]
    ...
    def save_multi_process(self, frames_clips, bvps_clips, filename):
        ...
        for i in range(len(bvps_clips)):
            ...
            input_path_name = self.cached_path + os.sep + "{0}_input{1}".format(filename, str(count)) + ".npy"
            label_path_name = self.cached_path + os.sep + "{0}_label{1}".format(filename, str(count)) + ".npy"
    ...
    def multi_process_manager(self, data_dirs, config_preprocess, multi_process_quota=4):
        ...
        Returns:
            file_list_dict(Dict): Dictionary containing information regarding processed data (path names)
                                  Each row is a list of input path names
        """
    ...
    def build_file_list(self, file_list_dict):
        ...
        # cascade all the file lists into "file_list"
        for process_num, file_paths in file_list_dict.items():
            file_list = file_list + file_paths

        if not file_list:
            raise ValueError(self.dataset_name, '[BaseLoader.build_file_list]: No files in file list')
    ...
    def build_file_list_retroactive(self, data_dirs, begin, end):
        ...
        # important change! make sure file list would be sorted here
        filename_list = sorted(list(set(filename_list)))  # ensure all indexes are unique and SORTED
    ...
    def diff_normalize_data(data):
        ...
        # Need to check the normalization std
        res = (data[1:] - data[:-1]) / (1e-7 + data[1:] + data[:-1])
        res = res / np.std(res)
        res = np.append(res, np.zeros_like(res[0:1]), axis=0)
        res[np.isnan(res)] = 0
        return res
